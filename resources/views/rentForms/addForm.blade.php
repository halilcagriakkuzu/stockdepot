            ...
            <form action="{{ action('App\Http\Controllers\RentFormController@addFormStore', ['id' => $rentForm->id, 'productId' => $product->id, 'active' => $active]) }}" method="post">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control @error('description') is-invalid @enderror" autocomplete="form-desc" type="text" name="description" id="description" value="{{ old('description') ?? '' }}" placeholder="Açıklama">
                        @error('description')
                        <span class="error invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
